...
class guide_mitms_assign_user_organisations_step extends guide_default_step {
    ## Return the content the step should display when it is the active step in a guide:
    function content_step_active () {
        return '<p>This step requires you to assign each user to an organisation.</p>
            <p>When you are ready to start open the positions tab on a user\'s profile.</p>
            <p>Refer to the ' .
    ...
    function content_step_pending () {
        return '<p>Assign an organisation to users allows you to assign each user to the appropriate part of the organisational structure.</p>';
    }
