...
function mitms_print_my_team_nav($return=false) {
    ...
    if (!empty($teammembers) && count($teammembers) > 0) {
        ...
        ';
        $returnstr .= '<p>' . mitms_captivate_popup('Video help with viewing your team\'s details', 'manager view') . '</p>';
    }
    return $returnstr;
 ...
 */
function mitms_migrate_old_report_builder_reports(&$result) {
...
/**
 * Wrapper for link_to_popup_window() which displays a window
 * suitable for a captivate video
 *
 * @param string $linktext Wording of link
 * @param string $name Name of captivate directory in /local/captivate/
 * @return string Code to produce popup link
 */
function mitms_captivate_popup($linktext, $name) {
    global $CFG;

    $linkurl = $CFG->wwwroot . '/local/captivate/' .
        $name . '/' . $name . '.htm';
    $title = $linktext;
    $popupname = preg_replace('/[^a-zA-Z0-9]/','', $name);
    $width = 700;
    $height = 900;

    $img = '<img src="' . $CFG->pixpath . '/t/movie.png"' .
        ' width="16" height="16" alt="Video file" /> ';
    return $img . link_to_popup_window($linkurl, $popupname, $linktext,
        $width, $height, $title, null, true);
}
